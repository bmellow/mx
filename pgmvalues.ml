...
(** get functions for the package context variable that is passed to 
    all functions emiting C.  This package context variable
    is a tuple containing the type symbol table, symbol table, current queue of C code
    and a tree which stores the needs relation between classes in the program 
*)
